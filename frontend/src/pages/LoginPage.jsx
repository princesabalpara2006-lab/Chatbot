...
const LoginPage = () => {
  const { loginUser, loading, authError } = useContext(AuthContext);
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      // Redirect to OTP verification with state containing email
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="space-y-1">
        ...
        <p className="text-xs text-slate-400">Provide credentials to enter the conversational system</p>
      </div>
      ...
      <p className="text-xs text-center text-slate-400">
        New to AetherTalk?{' '}
        <Link to="/register" className="text-primary-500 font-bold hover:underline">Create Account</Link>
      </p>
